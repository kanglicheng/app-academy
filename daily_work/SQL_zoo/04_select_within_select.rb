...
def population_constraint
  ...
  SELECT
    ...
    FROM countries
    WHERE countries.population > (SELECT population FROM countries WHERE name = 'Canada') AND
    countries.population < (SELECT population FROM countries WHERE name = 'Poland');
  SQL
end
...
def sparse_continents
  ...
  execute(<<-SQL)
  SELECT c.name, c.continent, c.population FROM countries c WHERE c.continent NOT IN (SELECT c2.continent FROM countries c2  WHERE c2.population >= 25000000)
